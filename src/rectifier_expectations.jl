...
M(α, μ, σ) = M(α*μ, abs(α)*σ)

M(α, b, μ, σ) = M(α*μ+b, abs(α)*σ)
...
B(α, b, μ, σ) = B(α*μ+b, abs(α)*σ)

B_alternative(α, b, μ, σ) = V(α*μ+b, abs(α)*σ) + M(α*μ+b, abs(α)*σ)^2 # used only for verification purposes
